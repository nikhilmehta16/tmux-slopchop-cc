...
import { Editor, type EditorTheme, Key, matchesKey, ProcessTerminal, truncateToWidth, TUI, visibleWidth, wrapTextWithAnsi } from "@earendil-works/pi-tui";
import { theme as defaultTheme, type Theme, type ThemeColor } from "../standalone/theme.js";
...
// Theme/ThemeColor are now concrete local types (see ../standalone/theme.ts).
// ReviewAppTheme is retained as an alias so existing exported references keep
// resolving.
export type ReviewAppTheme = Theme;
...
/**
 * Standalone entry point: build the TUI ourselves (no Pi `ctx`) and run the
 * review app to completion. Mirrors what Pi's `ctx.ui.custom` did — construct
 * the component with a `done` resolver, mount it, start the render/input loop,
 * and resolve when the user submits or cancels — then tear everything down.
 */
export async function runReviewAppStandalone(
  options: Omit<ReviewAppOptions, "notify"> & { notify?: Notify },
): Promise<{ result: ReviewResult; files: ReviewFile[] }> {
  const tui = new TUI(new ProcessTerminal());

  let resolveDone!: (value: { result: ReviewResult; files: ReviewFile[] }) => void;
  const donePromise = new Promise<{ result: ReviewResult; files: ReviewFile[] }>((resolve) => {
    resolveDone = resolve;
  });

  const app = new ReviewApp(tui, defaultTheme, resolveDone, {
    ...options,
